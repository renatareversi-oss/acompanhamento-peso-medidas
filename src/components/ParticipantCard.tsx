...
export function ParticipantCard({ participant }: { participant: Participant }) {
  ...
  return (
    <button
      ...
      className="group text-left w-full bg-white/80 backdrop-blur rounded-3xl p-5 shadow-[0_6px_20px_-8px_rgba(200,120,150,0.35)] border border-white hover:shadow-[0_10px_28px_-8px_rgba(200,120,150,0.5)] hover:-translate-y-1 transition-all duration-200 animate-pop-in cursor-pointer"
    >
        ...
        {delta != null && (
          <span
            className={`text-xs font-bold px-2.5 py-1 rounded-full ${
              delta < 0
                ? 'bg-[#d9f5e6] text-[#1d7a5f]'
                : delta > 0
                  ? 'bg-[#ffe1e1] text-[#c23764]'
                  : 'bg-[#f1f1f1] text-[#777]'
            }`}
          >
