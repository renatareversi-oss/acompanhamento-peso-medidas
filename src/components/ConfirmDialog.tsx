...
export function ConfirmDialog({ title, message, confirmLabel = 'Excluir', onConfirm, onCancel }: ConfirmDialogProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4" onClick={onCancel}>
      <div
        className="w-full max-w-sm bg-white rounded-3xl shadow-2xl p-6 animate-pop-in text-center"
      ...
      >
        ...
        <div className="flex gap-3 mt-5">
          <button
            onClick={onCancel}
            className="flex-1 rounded-2xl bg-[#f5f0f2] hover:bg-[#ece4e8] text-[var(--color-text)] font-semibold py-2.5 transition-colors"
          >
            Cancelar
          </button>
          <button
